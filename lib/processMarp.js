...
// PDF用のMarkdownコンテンツを準備（絶対パス使用）
function preparePdfMarkdown(content, originalFileDir, workspaceRoot) {
  const PATHSEP = "/"; // use "/" for file URI

  // マークダウンの画像記法を処理：相対パスを絶対パスに変換
  content = content.replace(/!\[([^\]]*)\]\(([^)"]*)(?: "([^"]*)")?\)/g, (match, alt, href, title) => {
    if (!href || href.startsWith("http") || href.startsWith("data:") || path.isAbsolute(href)) {
      return match;
    }
    ...
    try {
      ...
      if (fs.existsSync(absoluteImagePath)) {
        // Windowsのパス区切りを修正
        const fixedPath = absoluteImagePath.replace(/\\/g, "/");
        return `![${alt}](${fixedPath}${title ? ` "${title}"` : ""})`;
      }
    } catch (err) {
      // エラー時は元のパスを維持
    }
  ...
  // HTML imgタグを処理
  content = content.replace(/<(img|image)\s+([^>]*)src=["']([^"']*)["']([^>]*)>/gi, (match, tagName, beforeSrc, src, afterSrc) => {
    if (!src || src.startsWith("http") || src.startsWith("data:") || path.isAbsolute(src)) {
      return match;
    }

    try {
      const absoluteImagePath = path.resolve(originalFileDir, src);
      if (fs.existsSync(absoluteImagePath)) {
        const fixedPath = absoluteImagePath.replace(/\\/g, "/");
        return `<img ${beforeSrc}src="${fixedPath}" ${afterSrc}>`;
      }
    } catch (err) {
      // エラー時は元のsrcを維持
    }
...
async function processMarpFile(filePath, relativePath, marpSupport = true, cache = {}, stats = {}, isFileChanged, updateCache) {
  ...
  try {
    ...
    const ciOptions = timeoutSettings.isCI ? "--browser-arg=--no-sandbox --browser-arg=--disable-dev-shm-usage --browser-arg=--disable-gpu" : "";
    ...
    try {
      ...
      await executeWithRetry(htmlCmd); // HTML生成後にCSSリンクを追加
      ...
      try {
        await executeWithRetry(pdfCmd);
        console.log(`  ✅ PDF完了: ${path.basename(outputPdfPath)}`);
      } catch (pdfError) {
        console.error(`  ❌ PDF生成エラー: ${pdfError.message}`);
        if (getTimeoutSettings().isCI) {
          console.warn(`  ℹ️ CI環境でのPDF生成は時間がかかる場合があります`);
        }
      }
